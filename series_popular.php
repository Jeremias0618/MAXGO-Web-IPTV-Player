...
if ($output && is_array($output) && count($output) > 0) {
    // Elegir una serie aleatoria del listado
    $serie_aleatoria = $output[array_rand($output)];
    $serie_id = $serie_aleatoria['series_id'];
    // Llamar a get_series_info para obtener el backdrop_path (array de URLs)
...
if ($series_pagina && is_array($series_pagina)) {
    foreach($series_pagina as $index) {
        ...
        $serie_rat = $index['rating'];
        $avs = $index['rating_5based'];
        $serie_ano = isset($index['releaseDate']) ? $index['releaseDate'] : '';
?>
    <div class="col-6 col-sm-4 col-lg-3 col-xl-3">
        <div class="card">
            ...
            <span class="card__rate">
                <?php echo ($serie_ano ? substr($serie_ano, 0, 4) : ''); ?>
                &nbsp; <i class="fa-solid fa-star"></i><?php echo $serie_rat; ?>
            </span>
            </div>
        </div>
    </div>
<?php
    ...
    echo '<div style="color:#fff;font-size:1.2rem;">No hay series populares.</div>';
}
?>
        </div>
